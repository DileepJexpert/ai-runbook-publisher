...
def main(repo, base, head, url, review_mode, engine, coder_mode, output_root, no_fetch):
    if url:
    ...
    results=ReviewOrchestrator(IdfcCoderEngine(input_mode=coder_mode)).run(repo,base,head,ReviewMode(review_mode),output_root,fetch=not no_fetch)
