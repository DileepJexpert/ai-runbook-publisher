"""Deterministic Markdown-to-HTML and Confluence HTML generation for Production Support Runbooks."""
...
SCRIPT_TAG_PATTERN = re.compile(r"<\s*/?\s*script[^>]*>", re.IGNORECASE)
JAVASCRIPT_URI_PATTERN = re.compile(r"href\s*=\s*['\"]javascript:[^'\"]*['\"]", re.IGNORECASE)
EVENT_HANDLER_PATTERN = re.compile(r"\s+on\w+\s*=\s*['\"][^'\"]*['\"]", re.IGNORECASE)


def sanitize_html(html_text: str) -> str:
    """
    Sanitize HTML content by stripping executable script tags, javascript: URIs, and event handlers.
    Ensures safe rendering without arbitrary script execution.
    """
    if not html_text:
        return ""
    sanitized = SCRIPT_PATTERN.sub("", html_text)
    sanitized = SCRIPT_TAG_PATTERN.sub("", sanitized)
    sanitized = JAVASCRIPT_URI_PATTERN.sub('href="#"', sanitized)
...
def render_confluence_body(markdown_text: str) -> str:
    """
    Alias for render_body with Confluence safety sanitization.
    """
    return render_body(markdown_text)
...
def generate_runbook_html(
    ...
    """
    Deterministically generate confluence-body.html and RUNBOOK.html in output_dir from validated RUNBOOK.md.
    RUNBOOK.md is read-only and remains byte-for-byte untouched.
    Returns path to RUNBOOK.html.
    """
    ...
    # 1. Confluence body HTML
    confluence_body = render_confluence_body(markdown_content)
    confluence_target = out_dir / "confluence-body.html"
    confluence_target.write_text(confluence_body, encoding="utf-8")

    # 2. Standalone HTML document for local browser inspection
    ...
    LOGGER.info("Generated confluence-body.html and RUNBOOK.html at %s", out_dir)
    return html_target
